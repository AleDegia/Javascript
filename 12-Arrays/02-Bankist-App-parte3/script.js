...
//FIND Method
//Il metodo find in JavaScript viene utilizzato per trovare il primo elemento in un array che soddisfa una condizione specifica.
//Esso accetta una funzione di callback come parametro principale, che viene eseguita su ogni elemento dell'array fino a quando non
//trova un elemento per il quale la funzione di callback ritorna true  ->   callback(element, index, array):
const firstWithdrawal = movements.find(mov => mov<0)
console.log(firstWithdrawal);   //-400
console.log(accounts);          //4 oggetti con varie proprietà
const account = accounts.find(acc => acc.owner === 'Jessica Davis');    //se true ritorna l'elemento e interrompe il ciclo
console.log(account);           //{owner: 'Jessica Davis', movements: Array(8), interestRate: 1.5, pin: 2222, username: 'jd'}



//Implementiamo il Log In
let currentAccount;

btnLogin.addEventListener('click', function(e){
  e.preventDefault();   //previene che il form venga submittato e che la pagina venga di conseguenza ricaricata
  
  //trovo l'account con username che lo user ha messo nel form di login
  currentAccount = accounts.find(
    acc => acc.username === inputLoginUsername.value
  );
  console.log(currentAccount);

  //pin viene letta solo se il currentAccount esiste, per non dare errori
  if(currentAccount?.pin === Number(inputLoginPin.value)){  //verifico la password
