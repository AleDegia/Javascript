...
const poll = {
  ...
  answers: new Array(4).fill(0), //fill riempie tutti gli elementi dell'array con il valore specificato (0)
  registerNewAnswer: function(){
    let answer = prompt(" What is your favourite programming language? \n 0: JavaScript \n 1: Python \n 2: Rust \n 3: C++");
    poll.answers[Number(answer)] += 1;
...
const poll2 = {
  ...
  answers: new Array(4).fill(0), //fill riempie tutti gli elementi dell'array con il valore specificato (0)
  registerNewAnswer (){
    //get answer (unisco le stringhe di options separandole con l'a capo)
    const answer = Number(prompt(`${this.question}\n ${this.options.join('\n')}\n(Write option number)`));
    console.log(answer);

    //Register answer (salta i true ed esegue i null/undefined oppure l'ultimo)
    typeof answer === 'number' && answer < this.answers.length && this.answers[answer]++;
    this.displayResults();
    this.displayResults('string');
  },
  displayResults(type = 'array'){ //definizione metodo
    if(type === 'array') 
...
//la this keyword senza il bind punta al bottone poichè in un event handler function il this punta all elemento a cui è attaccato
document.querySelector('.poll').addEventListener('click', poll2.registerNewAnswer.bind(poll2));
...
/*
Unlike .bind(), which returns a new function with a bound this value,
